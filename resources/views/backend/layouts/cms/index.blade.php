                ...
                <div class="page-header">
                    <div>
                        <h1 class="page-title">CMS : {{ ucwords(str_replace('_', ' ', $page ?? '')) }} Page
                            {{ ucwords(str_replace('_', ' ', $section ?? '')) }} Section.</h1>
                    </div>
                    <div class="ms-auto pageheader-btn">
                        <button onclick="window.location.href=`{{ route($url . '.display') }}`"
                            class="btn me-2 {{ isset($data->is_display) && $data->is_display == 0 ? 'btn-danger' : 'btn-primary' }}">Display</button>
                    </div>
                </div>
                <!-- PAGE-HEADER END -->
                            ...
                            <div class="card-body">
                                <form class="form form-horizontal" method="POST" action="{{ route($url . '.content') }}"
                                    ...
                                    <div class="row {{ in_array('title', $component) ? '' : 'd-none' }}">
                                        <div class="col-md-12">
                                            <x-form.text name="title" label="Title" placeholder="Enter here title"
                                                :value="$data->title ?? ''" />
                                        </div>
                                    </div>
                                    ...
                                    <div class="row {{ in_array('sub_title', $component) ? '' : 'd-none' }}">
                                        <div class="col-md-12">
                                            <x-form.text name="sub_title" label="Sub Title"
                                                placeholder="Enter here sub title" :value="$data->sub_title ?? ''" />
                                        </div>
                                    </div>
                                    ...
                                    <div class="row {{ in_array('description', $component) ? '' : 'd-none' }}">
                                        <div class="col-md-12">
                                            <x-form.textarea name="description" label="Description"
                                                placeholder="Enter here description" :value="$data->description ?? ''" />
                                        </div>
                                    </div>
                                    ...
                                    <div class="row {{ in_array('sub_description', $component) ? '' : 'd-none' }}">
                                        <div class="col-md-12">
                                            <x-form.textarea name="sub_description" label="Sub Description"
                                                placeholder="Enter here sub description" :value="$data->sub_description ?? ''" />
                                        </div>
                                    </div>
                                            ...
                                            @if ($hasVideo)
                                                <div class="{{ $hasImage || $hasBg ? 'col-md-4' : 'col-md-12' }}">
                                                    <x-form.file name="video" label="Video" :file="$data->video ?? ''">
                                                        <p class="textTransform">Video Size must be less than 10MB. Allowed
                                                            formats: mp4, avi, mov.</p>
                                                    </x-form.file>
                                                </div>
                                            @endif
                                        </div>
